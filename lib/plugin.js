...
var createPreprocesor = function(resolver, basePath) {
  return function(content, file, done) {
    resolver.updateFile(file.originalPath, content);

    const isModule = content.match(/goog\.module\([\"\'].*[\"\']\)/);
    if (isModule) {
      const relativePath = path.relative(basePath, file.originalPath);
      const sourceMap = `//# sourceURL=http://closure/base/${relativePath}`;
      const contentStr = JSON.stringify(`${content}\n${sourceMap}`);
      content = `goog.loadModule(${contentStr})`;
    }
    done(content);
  };
};

createPreprocesor.$inject = ['x:resolver', 'config.basePath'];
